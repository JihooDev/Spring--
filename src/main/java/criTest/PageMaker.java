...
public class PageMaker {
	private int totalRowCount; // 전처 Row의 갯수 (전체 Page 수)
	private int spageNo; // Vieqw에 표시될 start Page no
	private int epageNo; // view에 표시될 end Page no
	// ※ 주의 필드명이 ePage 처럼 두번쨰 알파벳이 대문자인 경우
	// => setter, getter 는 setsPageNo , getsPageNo 형태로 만들어지기때문에
	// Lombok.. 등등 사용시 불편 -> 그러므로 대.소문자 섞어사용시 주의.

	private int displayPageNo = 3; // View에 표시될 Pageno 갯수
...
//	}
	...
	public void calcData() {
		// 2.1) sPageNo, ePageNo
		// 2.1) sPageNo, ePageNo
		// => currPage가 속한 페이지블럭의 ePageNo 를 계산
		...
		this.epageNo = (int) Math.ceil(cri.getCurrPage() / (double) this.displayPageNo) * this.displayPageNo;
		this.spageNo = (this.epageNo - this.displayPageNo) + 1;

		// 3.2) lastPageNo 계산, ePageNo 확인

		this.lastPageNo = (int) Math.ceil(this.totalRowCount / (double) cri.getRowsPerPage());

		if (this.epageNo > this.lastPageNo) {
			this.epageNo = this.lastPageNo;
		}

		this.prev = this.spageNo == 1 ? false : true;
		this.next = this.epageNo == this.lastPageNo ? false : true;
	}
	...
	public String searchQuery(int currPage) {
		UriComponents uriComponents = UriComponentsBuilder.newInstance().queryParam("currPage", currPage). // ?currPage=4
